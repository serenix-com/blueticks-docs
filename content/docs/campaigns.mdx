...
referenced variable, `onMissingVariable`
...
Two more optional fields — `mediaUrl` and its `mediaCaption` — attach an
image/video/document to every message. (The wire body is camelCase; the
Python, Ruby, PHP, and Go SDKs accept the snake_case forms shown in their tabs
...
The response `data` includes live counters (`sentCount`, `deliveredCount`,
`readCount`, `failedCount`) plus `status: "pending"`. (SDKs expose these under
their idiomatic names, e.g. Python's `cmp.sent_count`.)
...
console.log(cmp.status, cmp.sentCount, cmp.deliveredCount);
```
